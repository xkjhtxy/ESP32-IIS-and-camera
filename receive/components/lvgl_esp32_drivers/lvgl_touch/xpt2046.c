...
bool xpt2046_read(lv_indev_drv_t * drv, lv_indev_data_t * data)
{
	...
	if (irq == 0) {
		...
		if(TP_Read_XY2(&ux,&uy)==1){
			...
			if(ux > 370){ux -= 370;}else{ux = 0;}
			if(uy > 163){uy -= 163;}else{uy = 0;}
			ux = (uint32_t)((uint32_t)ux * LV_HOR_RES) / (3928 - 370);//320   3870-350
			uy = (uint32_t)((uint32_t)uy * LV_VER_RES) / (3865 - 163);// 240  3870-190
			// 翻转坐标
			//ux =  LV_HOR_RES - ux;
			//ux=ux + 20;
			//uy =  LV_VER_RES - uy;
